...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        let allCategories = await api.getTipoProductos();
        ...
        function renderCategories(cats) {
            ...
            if (cats.length === 0) {
                categoriesGrid.innerHTML = '<p class="text-slate-500 col-span-full text-center py-12">No se encontraron categorías.</p>';
                return;
            }
        ...
        if (searchInput) {
            searchInput.addEventListener('input', (e) => {
                ...
                const filtered = allCategories.filter(c => c.nombre.toLowerCase().includes(term));
